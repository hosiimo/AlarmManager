...
class AlarmManagerService extends IAlarmManager.Stub {
    ...
    private ArrayList<InFlight> mInFlight = new ArrayList<InFlight>();//最後にtriggerが合体するやつ？
    ...
    static final class Batch {
        ...
        boolean connect;//通信判定用（Alarmにも設定済み）
        ...
        boolean add(Alarm alarm) {
            ...
            if (DEBUG_BATCH) {
                Log.d(TAG, "Adding" + alarm.operation + alarm + "Alarm Time:" + alarm.when + " to " + this);
                //Log.i(TAG,"add new throwable",new Throwable());
            }
            ...
            if (DEBUG_BATCH) {
                ...
                for (int i = 0; i < alarms.size(); i++) {
                    Log.i(TAG, "This BATCH have " + alarms.get(i).operation + "ALARM TIME " + alarms.get(i).when);
                }
            }
            return newStart;
        }
    ...
    int attemptCoalesceLocked(long whenElapsed, long maxWhen, boolean connect) {
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            if (connect) {
                if (!b.standalone && b.canHold(whenElapsed, maxWhen) && b.connect) ;
                return i;
            } else {
        ...
        for (int batchNum = 0; batchNum < oldBatches; batchNum++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                setImplLocked(a.type, a.when, whenElapsed, a.windowLength, maxElapsed,
                        a.repeatInterval, a.operation, batch.standalone, doValidate, a.workSource, a.connect);
            }
        }
    }
    ...
    public AlarmManagerService(Context context) {
        ...
        if (mDescriptor != -1) {
            ...
            Log.d(TAG, "Failed to open alarm driver. Falling back to a handler.");
        }
    }
    ...
    public void set(int type, long triggerAtTime, long windowLength, long interval,
        ...
        set(type, triggerAtTime, windowLength, interval, operation, false, workSource, false);
    }

    //追加したAPI
    public void setConnect(int type, long triggerAtTime, long windowLength, long interval,
                           PendingIntent operation, WorkSource workSource, boolean connect) {
        set(type, triggerAtTime, windowLength, interval, operation, false, workSource, connect);
    }

    //isStandaloneがtrueの内部メソット用
    public void set(int type, long triggerAtTime, long windowLength, long interval,
                    PendingIntent operation, boolean isStandalone, WorkSource workSource) {
        set(type, triggerAtTime, windowLength, interval, operation, isStandalone, workSource, false);
    }

    public String exchange_milliseconde(long time) {

        SimpleDateFormat D = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
        Date Now = new Date();
        time = time + 100000L;
        String rTime = D.format(new Date(time));
        return rTime;
    }

    //setでmaxElapsedを設定してる（setの種類によって違う）
    public void set(int type, long triggerAtTime, long windowLength, long interval,
                    PendingIntent operation, boolean isStandalone, WorkSource workSource, boolean connect) {
        if (operation == null) {
            Log.d(TAG, "set/setRepeating ignored because there is no intent");
            return;
        }
        ...
        if (windowLength > AlarmManager.INTERVAL_HALF_DAY) {
            Log.d(TAG, "Window length " + windowLength
        ...
        if (triggerAtTime < 0) {
            ...
            Log.d(TAG, "Invalid alarm trigger time! " + triggerAtTime + " from uid=" + who
                    + " pid=" + what);
            triggerAtTime = 0;
        }
        ...
        final long maxElapsed;
        //もしもsetが精度高いやつだと。API19以下setおよびsetexact
        if (windowLength == AlarmManager.WINDOW_EXACT) {
            maxElapsed = triggerElapsed;
        } else if (windowLength < 0) {
            maxElapsed = maxTriggerTime(nowElapsed, triggerElapsed, interval);//setが不正確なやつ(public static final long WINDOW_HEURISTIC = -1;だと）
            //なんかトリガーが実行されるまでの*0.75倍の実行時間が追加されてる。つまりendまでの幅が大きい。
        } else {
        ...
        synchronized (mLock) {
            if (DEBUG_BATCH) {
                //typeは0=RTC_WAKEUP 1=RTC 2=ELAPSED_REALTIME_WAKEUP 3=ELAPSED_REALTIME
                //triggerAtTime=トリガーの最初の起動時間（実端末起動時間）,windowLengthは読込間隔？
                // tElapsedは???,maxElpsed=最大いつまでまてるか？
                //intervalは反復
                if (type > 1) {
                    Log.d(TAG, "setconnect[" + connect + "](" + operation + ") : type=" + type + " in minutes:" + ((triggerAtTime - nowElapsed) / 1000 / 60)
                            + " triggerAtTime=" + triggerAtTime + " win=" + windowLength
                            + " tElapsed=" + triggerElapsed + " maxElapsed=" + maxElapsed
                            + " interval=" + interval + " standalone=" + isStandalone);
                } else {
                    Log.d(TAG, "setconnect[" + connect + "](" + operation + ") : type=" + type + " in minutes:" + exchange_milliseconde(triggerAtTime)
                            + " triggerAtTime=" + triggerAtTime + " win=" + windowLength
                            + " tElapsed=" + triggerElapsed + " maxElapsed=" + maxElapsed
                            + " interval=" + interval + " standalone=" + isStandalone);
                }
            ...
            setImplLocked(type, triggerAtTime, triggerElapsed, windowLength, maxElapsed,
                    interval, operation, isStandalone, true, workSource, connect);
        }
    }

    //以下どのbatchにいれるかどうかを決めてる。
    private void setImplLocked(int type, long when, long whenElapsed, long windowLength,
                               long maxWhen, long interval, PendingIntent operation, boolean isStandalone,
                               boolean doValidate, WorkSource workSource, boolean connect) {
        Alarm a = new Alarm(type, when, whenElapsed, windowLength, maxWhen, interval,
                operation, workSource, connect);
        removeLocked(operation); //おなじPendingIntentがあった場合、mAlarmBatchesが再構成されるから超batch.addされる。

        int whichBatch = (isStandalone) ? -1 : attemptCoalesceLocked(whenElapsed, maxWhen, connect);//standaloneな独立してるものは単体バッチ、それ以外のもので、startがend早くかつエンドがstartよりも遅いバッチに合体
        if (whichBatch < 0) {
            Batch batch = new Batch(a);
            //Log.d(TAG,"new batch isStandalone["+isStandalone+"] "+"connect["+connect+"]"+a.operation+""+a+"Alarmtime:"+a.whenElapsed+"NEW BATCH:"+batch);
            batch.standalone = isStandalone;
            batch.connect = connect;
            addBatchLocked(mAlarmBatches, batch);//同じバッチなら上書き、それ以外なら開始時間順にソートして挿入される。
        } else {
            ...
            if (batch.add(a)) {
                ...
                addBatchLocked(mAlarmBatches, batch);//mAlarmbatchの時間軸順にbatchを追加
            }
        }

        if (DEBUG_VALIDATE) {
            if (doValidate && !validateConsistencyLocked()) {
                Log.d(TAG, "Tipping-point operation: type=" + type + " when=" + when
                        + " when(hex)=" + Long.toHexString(when)
                        + " whenElapsed=" + whenElapsed + " maxWhen=" + maxWhen
                        + " interval=" + interval + " op=" + operation
                        + " standalone=" + isStandalone);
                rebatchAllAlarmsLocked(false);//mAlarmBatchを作り直し？ただし一度起動するとdovalidataがfalseになる。そもそもDEBUG_VALIDATAがデフォルトでfalse
            }
        }
        ...
        if (DEBUG_VALIDATE) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (b.start >= lastTime) {
                    ...
                    Log.d(TAG, "CONSISTENCY FAILURE: Batch " + i + " is out of order");//LONG.MINよりも小さいという間違いない間違い。
                    logBatchesLocked();
                    return false;
                }
            }
    ...
    private void rescheduleKernelAlarmsLocked() {
        ...
        if (mAlarmBatches.size() > 0) {
            ...
            //もしwakeupを含んでいるbatchが存在し、かつmNextWakeupがそのバッチの時刻じゃなかったら。つまりmNextWakeupがまともに機能してたら再度登録されないはず。
            if (firstWakeup != null && mNextWakeup != firstWakeup.start) {
                mNextWakeup = firstWakeup.start;//mNextWakeupとmNextNonWakeupはdump用
                setLocked(ELAPSED_REALTIME_WAKEUP, firstWakeup.start);
            }
            //一番最初のバッチがwakeupじゃなくて、mNextNonWakeupがfirstBatchじゃなかったら。
        ...
        try {
            ...
            synchronized (this) {
                ...
                if (current == null || !current.equals(zone.getID())) {
                    if (localLOGV) {
                        Log.d(TAG, "timezone changed: " + current + ", new=" + zone.getID());
                    }
    ...
    public void removeLocked(PendingIntent operation) {
        ...
        if (didRemove) {
            if (DEBUG_BATCH) {
                Log.d(TAG, "remove(" + operation + ") changed bounds; rebatching");
            }
            rebatchAllAlarmsLocked(true);
            rescheduleKernelAlarmsLocked();
        }
    ...
    public void removeLocked(String packageName) {
        ...
        if (didRemove) {
            if (DEBUG_BATCH) {
                Log.d(TAG, "remove(package) changed bounds; rebatching");
            }
            rebatchAllAlarmsLocked(true);
            rescheduleKernelAlarmsLocked();
        }
    ...
    public void removeUserLocked(int userHandle) {
        ...
        if (didRemove) {
            if (DEBUG_BATCH) {
                Log.d(TAG, "remove(user) changed bounds; rebatching");
            }
            rebatchAllAlarmsLocked(true);
            rescheduleKernelAlarmsLocked();
        }
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (alarm.repeatInterval > 0) {
                    ...
                    setImplLocked(alarm.type, alarm.when + delta, nextElapsed, alarm.windowLength,
                            ...
                            alarm.workSource, alarm.connect);
                }
    ...
    private static class Alarm {
        ...
        public Alarm(int _type, long _when, long _whenElapsed, long _windowLength, long _maxWhen,
                     long _interval, PendingIntent _op, WorkSource _ws, Boolean _connect) {
            type = _type;
            when = _when;
        ...
        public void run() {
            ...
            while (true) {
                ...
                if ((result & TIME_CHANGED_MASK) != 0) {
                    if (DEBUG_BATCH) {
                        Log.d(TAG, "Time changed notification from kernel; rebatching");
                    }
                ...
                synchronized (mLock) {
                    ...
                    if (WAKEUP_STATS) {
                        if ((result & IS_WAKEUP_MASK) != 0) {
                            ...
                            for (int i = 0; i < n; i++) {
                                mRecentWakeups.remove();//古いやつは消してる
                            }

                            recordWakeupAlarms(mAlarmBatches, nowELAPSED, nowRTC);//dump用のやつ。時間が経過したmAlarmBatchのALARMを全部取り出しmRecentWakeupsに突っ込んでる
                        }
                    }
                    ...
                    for (int i = 0; i < triggerList.size(); i++) {
                        ...
                        try {
                            if (localLOGV || true){
                                if(alarm.operation.getIntent().getAction() != null) {
                                    action = alarm.operation.getIntent().getAction();
                                }else {
                                    action = alarm.operation.getIntent().getComponent().toShortString();
                                }
                            }
                            if(!action.equals("android.net.ConnectivityService.action.PKT_CNT_SAMPLE_INTERVAL_ELAPSED")){
                            ...
                            Log.d(TAG, "Failure sending alarm.", e);
                        }
                    }
                }
            }
        ...
        public void handleMessage(Message msg) {
            if (msg.what == ALARM_EVENT) {
                ...
                for (int i = 0; i < triggerList.size(); i++) {
                    Alarm alarm = triggerList.get(i);
                    try {
                        Log.d(TAG, "handlemessage send" + alarm.operation);
    ...
    class ClockReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(Intent.ACTION_TIME_TICK)) {
                if (DEBUG_BATCH) {
                    Log.d(TAG, "Received TIME_TICK alarm; rescheduling");
                }
        ...
        public void onSendFinished(PendingIntent pi, Intent intent, int resultCode,
            ...
            synchronized (mLock) {
                ...
                if (inflight != null) {
                    ...
                    Log.d("TAG", "No in-flight alarm for " + pi + " " + intent);
                }
                mBroadcastRefCount--;
                if (mBroadcastRefCount == 0) {
                    mWakeLock.release();
                    if (mInFlight.size() > 0) {
                        Log.d(TAG, "Finished all broadcasts with " + mInFlight.size()
                                + " remaining inflights");
                        for (int i = 0; i < mInFlight.size(); i++) {
                            Log.d(TAG, "  Remaining #" + i + ": " + mInFlight.get(i));
                        }
                        mInFlight.clear();
                    }
                } else {
                    ...
                    if (mInFlight.size() > 0) {
                        ...
                        // should never happen
                        Log.d(TAG, "Alarm wakelock still held but sent queue empty");
                        mWakeLock.setWorkSource(null);
                    }
                }
            }
