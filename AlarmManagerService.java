...
class AlarmManagerService extends IAlarmManager.Stub {
    ...
    private static final boolean DEBUG_CONNECT = true;
    ...
    private static final String CONNECT_ALARM = "com.google,line,facebook,twitter,poll_alarm_ev";
    ...
    private final SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss:SS");
    ...
    static final class Batch {
        ...
        boolean matchConnectBatch;//接続する用のバッチ
        ...
        boolean add(Alarm alarm) {
            ...
            if (alarm.matchConnectBatch) {
                matchConnectBatch = true;
            }
    ...
    int attemptCoalesceLocked(long whenElapsed, long maxWhen, boolean matchConnectBatch) {
        final int N = mAlarmBatches.size();
        for (int i = 0; i < N; i++) {
            Batch b = mAlarmBatches.get(i);
            if (matchConnectBatch) {
                if (!b.standalone && b.canHold(whenElapsed, maxWhen) && b.matchConnectBatch) {
                    Log.d(TAG, "add connect list");
                    return i;
                }
            } else {
                if (!b.standalone && b.canHold(whenElapsed, maxWhen) && !b.matchConnectBatch) {
        ...
        for (int batchNum = 0; batchNum < oldBatches; batchNum++) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                setImplLocked(a.type, a.when, whenElapsed, a.windowLength, maxElapsed,
                        a.repeatInterval, a.operation, batch.standalone, doValidate, a.workSource, a.matchConnectKey, false);
            }
        }
    }

    public void rebatchAllmConnectBatch() {
        ArrayList<Batch> oldSet = (ArrayList<Batch>) mConnectBatch.clone();
        mConnectBatch.clear();
        final int oldBatches = oldSet.size();
        for (int batchNum = 0; batchNum < oldBatches; batchNum++) {
            Batch batch = oldSet.get(batchNum);
            ...
            for (int i = 0; i < N; i++) {
                Alarm a = batch.get(i);
                a.matchConnectBatch = false;
                attemptCheckConnectAlarm(a);
                Log.d(TAG, "rebatchAllmConnectBatch");
            }
        }
    }
    ...
    public void set(int type, long triggerAtTime, long windowLength, long interval,
        ...
        synchronized (mLock) {
            ...
            boolean matchConnectKey = false;
            if (DEBUG_CONNECT) {
                if (checkConnectAlarm(operation)) {
                    matchConnectKey = true;
                    Log.d(TAG, "match connect Alarm");
                }
            }
            setImplLocked(type, triggerAtTime, triggerElapsed, windowLength, maxElapsed,
                    interval, operation, isStandalone, true, workSource, matchConnectKey, false);
        }
    }

    private final ArrayList<String> mSerchConnectAlarmKey = new ArrayList<String>();//特定のキーワードalarm判定用
    private final ArrayList<Batch> mConnectBatch = new ArrayList<Batch>();//通信すると思われるAlarmを格納したBatchが入ってる配列

    private final boolean checkConnectAlarm(PendingIntent operation) {
        //該当キーワードに引っかかるかチェック
        String action = giveActionString(operation);
        boolean result;
        if (mSerchConnectAlarmKey.isEmpty()) {
            String[] keys = CONNECT_ALARM.split(",");
            for (String key : keys) {
                Log.d(TAG, "alarm_key:" + key);
                mSerchConnectAlarmKey.add(key);
            }
        }
        for (int i = 0; mSerchConnectAlarmKey.size() > i; i++) {
            if (action.indexOf(mSerchConnectAlarmKey.get(i)) != -1) return true;
        }
        return false;
    }

    private final boolean attemptCheckConnectAlarm(Alarm alarm) {
        //通信用仮Batchに突っ込んで見る。
        //すでにあるBatchとうまくくっつくなら、Batch内を再度飛ばして、trueを返す。
        boolean result = false;


        for (int j = 0; mConnectBatch.size() > j; j++) {
            Batch b = mConnectBatch.get(j);
            if (b.canHold(alarm.whenElapsed, alarm.maxWhen)) {
                int N = b.size();
                alarm.matchConnectBatch = true;
                for (int i = 0; i < N; i++) {
                    Alarm a = b.get(i);
                    if (!a.matchConnectBatch) {
                        removeLockedforSetImplLocked(a.operation);
                        b.remove(a.operation);
                        setImplLocked(a.type, a.when, a.whenElapsed, a.windowLength, a.maxWhen, a.repeatInterval,
                                a.operation, b.standalone, true, a.workSource, a.matchConnectKey, true);
                        Log.d(TAG, "new match ConnectBatch Alarm set");
                    }
                }
                b.add(alarm);
                mConnectBatch.remove(b);
                addBatchLocked(mConnectBatch, b);
                Log.d(TAG, "new connect Alarm set");
                result = true;
            }
        }
        if (!result) {
            ...
            batch.matchConnectBatch = false;
            addBatchLocked(mConnectBatch, batch);
        }

        return result;
    }

    private final String giveActionString(PendingIntent operation) {
        String action = "";
        if (operation.getIntent().getAction() != null) {
            action = operation.getIntent().getAction();
        } else {
            action = operation.getIntent().getComponent().toShortString();
        }
        return action;
    }
    ...
    private void setImplLocked(int type, long when, long whenElapsed, long windowLength,
                               ...
                               boolean doValidate, WorkSource workSource, boolean matchConnectKey, boolean matchConnectBatch) {

        Alarm a = new Alarm(type, when, whenElapsed, windowLength, maxWhen, interval,
                operation, workSource, matchConnectKey, matchConnectBatch);

        if (DEBUG_CONNECT && matchConnectKey && !matchConnectBatch) {
            if (attemptCheckConnectAlarm(a)) {
                Log.d(TAG, "match Connect Batch");
            }
        }

        removeLockedforSetImplLocked(operation);
        //alarmがAlarmLaucherだった時はisStandaloneにする。
        String action = giveActionString(operation);
        if (action.indexOf("AlarmLaucher") != -1) {
            isStandalone = true;
        }
        //ここまで
        int whichBatch = (isStandalone) ? -1 : attemptCoalesceLocked(whenElapsed, maxWhen, a.matchConnectBatch);
        if (whichBatch < 0) {
            Batch batch = new Batch(a);
            batch.standalone = isStandalone;
            batch.matchConnectBatch = a.matchConnectBatch;
    ...
    public void removeMConnectBatch(PendingIntent operation) {
        //mConnectBatch再構成用
        boolean didRemove = false;
        for (Batch b : mConnectBatch) {
            didRemove |= b.remove(operation);
            if (b.size() == 0) {
                mConnectBatch.remove(b);
            }
        ...
        if (didRemove) {
            rebatchAllmConnectBatch();
        }
    }

    public void removeMConnectBatch(String packageName) {
        //mConnectBatch再構成用
        boolean didRemove = false;
        for (Batch b : mConnectBatch) {
    ...
    public void removeMConnectBatch(int userHandle) {
        //mConnectBatch再構成用
        boolean didRemove = false;
        for (Batch b : mConnectBatch) {
    ...
    public void removeLockedforSetImplLocked(PendingIntent operation) {
        //setImplLocked用のremoveLock。これ経由しないと、無限ループになるでー
        boolean didRemove = false;
        for (int i = mAlarmBatches.size() - 1; i >= 0; i--) {
            Batch b = mAlarmBatches.get(i);
            didRemove |= b.remove(operation);
    ...
    public void removeLocked(String packageName) {
        ...
        removeMConnectBatch(packageName);
    ...
    public void removeUserLocked(int userHandle) {
        ...
        removeMConnectBatch(userHandle);
    ...
    private class AddConectArrayList<T> extends ArrayList<T> {
        public boolean matchConnectBatch = false;
        public long start = 0;
        public long end = 0;
    }

    private void triggerAlarmsLocked(AddConectArrayList<Alarm> triggerList, long nowELAPSED, long nowRTC) {
        ...
        while (mAlarmBatches.size() > 0) {
            ...
            triggerList.matchConnectBatch = batch.matchConnectBatch;
            triggerList.start = batch.start;
            triggerList.end = batch.end;
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (alarm.repeatInterval > 0) {
                    ...
                    setImplLocked(alarm.type, alarm.when + delta, nextElapsed, alarm.windowLength,
                            ...
                            alarm.workSource, alarm.matchConnectKey, false);
                }
    ...
    private static class Alarm {
        ...
        public boolean matchConnectKey;//
        public boolean matchConnectBatch;

        public Alarm(int _type, long _when, long _whenElapsed, long _windowLength, long _maxWhen,
                     long _interval, PendingIntent _op, WorkSource _ws, boolean _matchConnectKey, boolean _matchConnectBacth) {
            type = _type;
            when = _when;
            ...
            operation = _op;
            workSource = _ws;
            matchConnectKey = _matchConnectKey;
            matchConnectBatch = _matchConnectBacth;
        ...
        public void dump(PrintWriter pw, String prefix, long now) {
            ...
            pw.print(whenElapsed);
            pw.print(" when=");
            TimeUtils.formatDuration(when, now, pw);
            pw.print(" window=");
            pw.print(windowLength);
    ...
    private boolean sendActionBroadcast(AddConectArrayList<Alarm> triggerList, String call) {
        if (triggerList.matchConnectBatch) {
            Intent alarm_check = new Intent("Intent.AlarmLaucher.Alarm_Check");
            String notificationAction = "--" + call + "通信処理BATCH------," + sdf.format(new Date(triggerList.start + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + "," + sdf.format(new Date(triggerList.end + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + ",Batch size:" + triggerList.size();
            alarm_check.putExtra("alarmname", notificationAction);
            mContext.sendBroadcast(alarm_check);
            return true;
        } else {
            boolean res = false;
            for (Alarm a : triggerList) {
                if (giveActionString(a.operation).indexOf("AlarmLaucher") != -1) {
                    res = true;
                }
            }
            if (!res && triggerList.size() > 0) {
                Intent alarm_check = new Intent("Intent.AlarmLaucher.Alarm_Check");
                String notificationAction = "--" + call + "BATCH処理------," + sdf.format(new Date(triggerList.start + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + "," + sdf.format(new Date(triggerList.end + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + ",Batch size:" + triggerList.size();
                alarm_check.putExtra("alarmname", notificationAction);
                mContext.sendBroadcast(alarm_check);
                return true;
            }
    ...
    private class AlarmThread extends Thread {
        ...
        public void run() {
            AddConectArrayList<Alarm> triggerList = new AddConectArrayList<Alarm>();
            ...
            while (true) {
                ...
                synchronized (mLock) {
                    ...
                    //追加した箇所
                    sendActionBroadcast(triggerList, "therad");
                    // now deliver the alarm intents
                    for (int i = 0; i < triggerList.size(); i++) {
                        Alarm alarm = triggerList.get(i);
                        try {
                            String action = giveActionString(alarm.operation);
                            if (action.indexOf("AlarmLaucher") == -1) {
                                Log.d(TAG, "sending " + action + " Alarm:" + alarm + "Alarmtime:" + alarm.whenElapsed);
                                Intent alarm_check = new Intent("Intent.AlarmLaucher.Alarm_Check");
                                action += "," + sdf.format(new Date(alarm.whenElapsed + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + "," + sdf.format(new Date(alarm.maxWhen + System.currentTimeMillis() - SystemClock.elapsedRealtime())) + "," + alarm.windowLength;
                                alarm_check.putExtra("alarmname", action);
                                mContext.sendBroadcast(alarm_check);//監視用にブロードキャスト
                            }
                            for (int k = mConnectBatch.size()-1; k > -1; k--) {
                                Batch b = mConnectBatch.get(k);
                                if (b.remove(alarm.operation)) {
                                    if (b.size() == 0) {
                                        mConnectBatch.remove(b);
                                        Log.d(TAG, "mConnectBatch remove");
                                    }
                                    Log.d(TAG, "mConnectBatch-Alarm remove");
                                }
                            }
                            //ここまで
        ...
        public void handleMessage(Message msg) {
            if (msg.what == ALARM_EVENT) {
                ...
                synchronized (mLock) {
                    ...
                    final long nowELAPSED = SystemClock.elapsedRealtime();
                    triggerAlarmsLocked(triggerList, nowELAPSED, nowRTC);
                    sendActionBroadcast(triggerList, "handler");
                }
